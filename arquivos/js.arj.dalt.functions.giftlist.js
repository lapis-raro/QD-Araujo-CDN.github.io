...
try {
	var List = {
		...
		addLinkBeforeSidebar: function() {
			...
			listLinkHtml[0].pathname = '/list/' + location.pathname.split('/')[2];
		...
		getGiftListCreateModalOpen: function() {
			...
			if (!$(document.body).is('.giftlist-manage') || (document.referrer.indexOf('/giftlist/create') < 0))
				return;
			...
			window.ShowInsertedSuccess = function() {
				...
				setTimeout(function () {
					if(listId)
						...
						window.location = "/giftlist/product?" + (($("#TB_window .glis-edit-link").attr('href') || '').match(/(\?)?(\&)?(id=[0-9]*)/i) || ['']).pop() + "&redirect=false";
		...
		formEnhancedValidation: function() {
			...
			window.validationForm = function() {
				...
				var sucesso2 = true;
				var fields = address.find('.postal-code');
				var fieldsButton = address.find('.save-address');
				if(fieldsButton.is('.giftlisterror')) {
					sucesso2 = false;
					fields.addClass('giftlisterror');
				}
				else {
					fields.removeClass('giftlisterror');
				}
