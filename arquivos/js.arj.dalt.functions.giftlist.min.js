...
a.ip},error:function(){$.ajax({url:"//www.telize.com/jsonip",dataType:"jsonp",success:function(a){c=a.ip}})}});var e=function(a){return $.ajax({url:"//api.vtexcrm.com.br/araujo/dataentities/LC/documents",type:"POST",dataType:"json",headers:{Accept:"json","Content-Type":"application/json; charset=utf-8"},data:JSON.stringify(a),success:function(a){b=a.Id.replace("LC-","")}})},d=function(b,e){var d=$(".qd-invite-lists .list-link option:selected");a=$(".gf-invite-block-3-models-invite .active-model").html()||
"";return{ip:c,email:b,user:e,message:a,listName:d.text(),listId:d.attr("data-qd-list-id")||null,listIco:$(".gf-invite-img-model.active-model img:first").attr("src")||null}},g=function(a){$.getJSON("/no-cache/profileSystem/getProfile").done(function(b){$.ajax({url:"//api.vtexcrm.com.br/araujo/dataentities/CL/search?userId="+b.UserId+"&_fields=id",dataType:"json",headers:{Accept:"application/vnd.vtex.ds.v10+json","Content-Type":"application/json; charset=utf-8"},error:function(){alert("Desculpe! N\u00e3o foi poss\u00edvel enviar seu formul\u00e1rio!")}}).done(function(b){a(b[0].id)})}).fail(function(){alert("Desculpe! N\u00e3o foi poss\u00edvel compartilhar sua lista!")})};
$(".qd-social-facebook, .qd-social-whatsapp").click(function(c){c.preventDefault();var f=$(this);if(($(".qd-invite-lists .list-link").val()||"").length){var h=function(){f.is(".qd-social-facebook")?window.open("https://www.facebook.com/sharer/sharer.php?u="+encodeURIComponent("http://chadebebe.araujo.com.br/convite/"+b),"fbShareWindow","height=300, width=400, top="+($(window).height()/2-275)+", left="+($(window).width()/2-225)+", toolbar=0, location=0, menubar=0, directories=0, scrollbars=0"):window.open("whatsapp://send?text="+
encodeURIComponent("http://chadebebe.araujo.com.br/convite/"+b),"whatsappShareWindow")};a!=($(".gf-invite-block-3-models-invite .active-model").html()||"")?g(function(a){e(d(null,a)).done(h).fail(function(){alert("Desculpe, n\u00e3o foi poss\u00edvel compartilhar sua lista!")})}):h()}else alert("Selecione o Link da lista.")});var f=$(".modal");$(".qd-social-email").click(function(b){b.preventDefault();if(($(".qd-invite-lists .list-link").val()||"").length){b=f.find(".modal-header");b.children(":not(button)").remove();
b.prepend('<h4 class="pull-left">Para quem deseja enviar?</h4>');f.modal();f.addClass("qd-invite-modal");var c=$('<div class="qd-send-invite"></div>').appendTo(f.find(".modal-body").empty());c.html("<p>Insira os e-mails (separe com v\u00edrgula ou espa\u00e7o):</p>");var h=$("<textarea></textarea>").appendTo(c).tagEditor({delimiter:" ,;"});$('<button class="btn">Enviar convite</button>').click(function(){var b=(h.val()||"").match(/([a-zA-Z0-9._\-\+])+@([a-z0-9._\-\+])+\.[a-z]{2,6}/g).join(",");if(!b.length)return alert("Preencha seu e-mail");
c.addClass("qd-invite-loading");g(function(g){var f=a!=($(".gf-invite-block-3-models-invite .active-model").html()||"");g=d(b,g);f&&e(g);$.ajax({url:"https://ssl-202418.kinghost.net/send-invite-cha-de-bebe/general_email.php",type:"POST",dataType:"json",data:g,success:function(){c.find(".form-succes").removeClass("hide")},complete:function(){c.removeClass("qd-invite-loading")}})})}).appendTo(c);$('<div class="form-succes green-alert hide"> <i class="fa fa-check-circle-o padding-r-sm font-size-sm"></i>Email enviado com sucesso </div>').appendTo(c)}else alert("Selecione o Link da lista.")})}},
...
getGiftBalance:function(){$.qdAjax({url:"/no-cache/profileSystem/getProfile",dataType:"json",clearQueueDelay:null,success:function(a){try{if(!a.IsUserDefined)return alert("Voc\u00ea precisa se logar para ver o saldo do vale.");$.ajax({url:"https://ssl-202418.kinghost.net/gift-card/gift-card.php?customerId="+a.UserId,dataType:"json",success:function(a){try{for(var b=0,e=0;e<a.items.length;e++)b+=a.items[e].balance;List.giftBalance="R$ "+qd_number_format(b/100,2,",",".")}catch(d){"undefined"!==typeof console&&
"function"===typeof console.error&&console.error("Problemas :( . Detalhes: ",d)}},error:function(){alert("Desculpe, infelizmente n\u00e3o foi poss\u00edvel consultar seu saldo no momento!")}})}catch(b){"undefined"!==typeof console&&"function"===typeof console.error&&console.error("Problemas :( . Detalhes: ",b)}},error:function(){alert("Desculpe, n\u00e3o foi poss\u00edvel consultar seu saldo no momento!")}})},changeReadyListTitle:function(){$(document.body).is(".giftlist-shelf")&&$(".giftlistinfo-title").text($(".giftlistinfo-description").remove().text())},
giftlistReadySetId:function(){var a=(location.search.match(/qdlistid=([0-9]+)/i)||[""]).pop();a.length&&$(document.body).is(".giftlist-ready")&&$('a[href^="/list/"]').each(function(){this.hash=this.hash+"&qdlistid="+a})},formEnhancedValidation:function(){var a=window.validationForm,b=$("#addresscomponent");window.validationForm=function(){a.apply(this,arguments);var c=!0,e=b.find(".postal-code");b.find(".save-address").is(".giftlisterror")?(c=!1,e.addClass("giftlisterror")):e.removeClass("giftlisterror");
return c}},fillAddressInformation:function(){var a=$(".address-form-new");a.find(".ship-number label span:not(:contains(*))").append(" *");a.find(".save-address").on("click",function(){0==a.find(".ship-number input").val().length?a.find(".ship-number input").addClass("error"):a.find(".ship-number input").removeClass("error")});var b=a.find("input#ship-postal-code");b.keyup(function(c){8>(b.val()||"").length||$.ajax({url:"/api/checkout/pub/postal-code/BRA/"+b.val(),dataType:"json",success:function(b){a.find("#ship-street").val(b.street||
"");a.find("#ship-neighborhood").val(b.neighborhood||"");a.find("#ship-city").val(b.city||"");a.find("#ship-state").val(b.state||"")}})})},getUserInfo:function(){function a(a){$(".panel-group .panel #collapsePreRegister");d.vtexToken=$(a).find("input#VTEXRequestVerificationToken").val();d.userId=$(a).find("input#userId").val();d.email=$(a).find("#editar-perfil input#email").val();d.document=$(a).find("#editar-perfil input#document").val();d.firstName=$(a).find("#editar-perfil input#firstName").val();
d.lastName=$(a).find("#editar-perfil input#lastName").val();d.homePhone=$(a).find("#editar-perfil input#homePhone").val();d.gender=$(a).find("#editar-perfil input[name=gender][checked=true]").val();d.email&&d.document&&d.firstName&&d.lastName&&(b=!0);b&&c?($(".panel-group .panel:first-child").addClass("qd_step_done").find("#collapsePreRegister .panel-body").empty().html('<h4 class="center">Etapa concluida!</h4>').addClass("bg-success"),$(".panel-group .panel #collapseCreateList").addClass("in")):
($(".panel-heading#createList a").attr("href","#"),a=$(".panel-group .panel #collapsePreRegister"),a.find("input[name=qd_form_name]").val(d.firstName||""),a.find("input[name=qd_form_lastname]").val(d.lastName||""),a.find("input[name=qd_form_email]").val(d.email),a.find("input[name=qd_form_cpf]").val(d.document||""),a.find("input[name=qd_form_phone]").val(d.homePhone||""),a.find("select[name=qd_form_gender]").val(d.gender||""),List.applyMasks(),List.fillAddressInfo(),List.enableDataSaveButton(d,e))}
var b=!1,c=!1,e={},d={};$.qdAjax({url:"/no-cache/giftlistv2/address/get/",dataType:"json",success:function(b){0<b.shippingData.availableAddresses.length&&(c=!0);e=b.shippingData.address;$.ajax({url:"/account",dataType:"html"}).done(function(b){a(b)}).error(function(b,c,d){a(b.responseText)})}})},enableDataSaveButton:function(a,b){var c=$(".panel-group .panel #collapsePreRegister");$(".giftlist-qd-v1-form-submit").on("click",function(b){b.preventDefault();var d=!1;c.find("input.required:not([disabled]), select").each(function(){0==
$(this).val().length&&($(this).focus(),d=!0)});console.log(d);d||(profileData={email:a.email,userId:a.userId,VTEXRequestVerificationToken:a.vtexToken,firstName:c.find("input[name=qd_form_name]").val(),lastName:c.find("input[name=qd_form_lastname]").val(),document:c.find("input[name=qd_form_cpf]").val(),homePhone:c.find("input[name=qd_form_phone]").val(),gender:c.find("select[name=qd_form_gender]").val(),isCorporate:"False"},shippingData={userId:a.userId,postalCode:c.find("input[name=qd_form_zipcode]").val(),
street:c.find("input[name=qd_form_street]").val(),number:c.find("input[name=qd_form_number]").val(),complement:c.find("input[name=qd_form_complement]").val()||"",neighborhood:c.find("input[name=qd_form_neighboor]").val(),city:c.find("input[name=qd_form_city]").val(),state:c.find("input[name=qd_form_state]").val(),receiverName:profileData.firstName,addressName:"Endereco",addressType:"1",reference:"",country:"BRA",addressId:""},e(profileData,shippingData))});var e=function(a,b){var c=0;c++;$.ajax({url:"/no-cache/account/profile/save",
type:"POST",data:a}).complete(function(){c--;0==c&&location.reload()});c++;$.ajax({url:"/no-cache/account/address/save",type:"POST",data:b}).complete(function(){c--;0==c&&location.reload()})}},fillAddressInfo:function(){var a=$(".panel-group .panel #collapsePreRegister"),b=a.find("input[name=qd_form_zipcode]");b.keyup(function(c){9>(b.val().replace("_","")||"").length||$.ajax({url:"/api/checkout/pub/postal-code/BRA/"+b.val(),dataType:"json",success:function(b){a.find("input[name=qd_form_street]").val(b.street||
"");a.find("input[name=qd_form_neighboor]").val(b.neighborhood||"");a.find("input[name=qd_form_city]").val(b.city||"");a.find("input[name=qd_form_state]").val(b.state||"");a.find("[disabled]:not(#qd_form_email), .enabled").each(function(){0==$(this).val().length?$(this).removeAttr("disabled").addClass("enabled"):$(this).attr("disabled","true").removeClass("enabled")})},error:function(){a.find("*[disabled]:not(#qd_form_email)").each(function(){$(this).removeAttr("disabled")})}})})},applyMasks:function(){$("#qd_form_cpf").mask("999.999.999-99");
$("#qd_form_phone").mask("(99) 9999-9999?9");$("#qd_form_zipcode").mask("99999-999")}};
try{(function(){var a;var b=function(){a.is(".giftlist")&&List.windowOnload()};var c=function(){a.is(".giftlist")&&List.ajaxStop()};$(function(){a=$(document.body);a.is(".giftlist")&&List.init();$(document).ajaxStop(c);$(window).load(b);a.addClass("jsFullLoadedGiftList")});List.run()})()}catch(a){"undefined"!==typeof console&&"function"===typeof console.error&&$("body").addClass("jsFullLoaded jsFullLoadedError")&&console.error("Houve um erro ao iniciar os objetos. Detalhes: "+a.message)};