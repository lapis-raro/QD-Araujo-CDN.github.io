...
$(".panel-group .panel #collapseCreateList").addClass("in")):(b=$(".panel-group .panel #collapsePreRegister"),b.find("input[name=qd_form_email]").val(a.Email),b.find("input[name=qd_form_name]").val(a.FirstName||""),b.find("input[name=qd_form_lastname]").val(a.LastName||""),List.applyMasks(),List.savePreRegister(a.Email,a.UserId))}})}})},savePreRegister:function(a,b){var c=$(".panel-group .panel #collapsePreRegister");$(".giftlist-qd-v1-form-submit").on("click",function(f){f.preventDefault();var h=
!1;c.find("input.required:not([disabled]), select").each(function(){0==$(this).val().length&&($(this).focus(),h=!0)});h||(profileData={email:a,userId:b,VTEXRequestVerificationToken:"",firstName:c.find("input[name=qd_form_name]").val(),lastName:c.find("input[name=qd_form_lastname]").val(),document:c.find("input[name=qd_form_cpf]").val(),homePhone:c.find("input[name=qd_form_phone]").val(),gender:c.find("select[name=qd_form_gender]").val(),businessPhone:"",isCorporate:"False",nickName:"",birthDate:""},
shippingData={userId:b,postalCode:c.find("input[name=qd_form_zipcode]").val(),street:c.find("input[name=qd_form_street]").val(),number:c.find("input[name=qd_form_number]").val(),complement:c.find("input[name=qd_form_complement]").val()||"",neighborhood:c.find("input[name=qd_form_neighboor]").val(),city:c.find("input[name=qd_form_city]").val(),state:c.find("input[name=qd_form_state]").val(),receiverName:profileData.firstName,addressName:"Endereco",addressType:"1",reference:"",country:"BRA",addressId:""},
d(profileData,shippingData))});var d=function(a,b){function c(c){a.VTEXRequestVerificationToken=c;d++;$.ajax({url:"/no-cache/account/profile/save",type:"POST",data:a}).complete(function(){d--;0==d&&location.reload()});d++;$.ajax({url:"/no-cache/account/address/save",type:"POST",data:b}).complete(function(){d--;0==d&&location.reload()})}var d=0;$.ajax({url:"/account",dataType:"html"}).done(function(a){c($(a).find("input#VTEXRequestVerificationToken").val()||"")}).error(function(a,b,d){c($(a.responseText).find("input#VTEXRequestVerificationToken").val()||
"")})}},fillAddressInfo:function(){var a=$(".panel-group .panel #collapsePreRegister .form-qd-v1-second-wrap"),b=a.find("input[name=qd_form_zipcode]");b.keyup(function(c){9>(b.val().replace("_","")||"").length||$.ajax({url:"/api/checkout/pub/postal-code/BRA/"+b.val(),dataType:"json",success:function(b){a.find("input[name=qd_form_street]").val(b.street||"");a.find("input[name=qd_form_neighboor]").val(b.neighborhood||"");a.find("input[name=qd_form_city]").val(b.city||"");a.find("input[name=qd_form_state]").val(b.state||
"");a.find("[disabled], .enabled").each(function(){0==$(this).val().length?$(this).removeAttr("disabled").addClass("enabled"):$(this).attr("disabled","true").removeClass("enabled")})},error:function(){a.find("*[disabled]").each(function(){$(this).removeAttr("disabled")})}})})},applyMasks:function(){$("#qd_form_cpf").mask("999.999.999-99");$("#qd_form_phone").mask("(99) 9999-9999");$("#qd_form_zipcode").mask("99999-999")}};
