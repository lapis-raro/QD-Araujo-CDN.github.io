...
encodeURIComponent("http://chadebebe.araujo.com.br/convite/"+b),"whatsappShareWindow")};a!=($(".gf-invite-block-3-models-invite .active-model").html()||"")?h(function(a){d(f(null,a)).done(e).fail(function(){alert("Desculpe, n\u00e3o foi poss\u00edvel compartilhar sua lista!")})}):e()}else alert("Selecione o Link da lista.")});var e=$(".modal");$(".qd-social-email").click(function(b){b.preventDefault();if(($(".qd-invite-lists .list-link").val()||"").length){b=e.find(".modal-header");b.children(":not(button)").remove();
b.prepend('<h4 class="pull-left">Para quem deseja enviar?</h4>');e.modal();e.addClass("qd-invite-modal");var c=$('<div class="qd-send-invite"></div>').appendTo(e.find(".modal-body").empty());c.html("<p>Insira os e-mails (separe com v\u00edrgula ou espa\u00e7o):</p>");var k=$("<textarea></textarea>").appendTo(c).tagEditor({delimiter:" ,;"});$('<button class="btn">Enviar convite</button>').click(function(){var b=(k.val()||"").match(/([a-zA-Z0-9._\-\+])+@([a-z0-9._\-\+])+\.[a-z]{2,6}/g).join(",");if(!b.length)return alert("Preencha seu e-mail");
...
"");a.find("#ship-neighborhood").val(b.neighborhood||"");a.find("#ship-city").val(b.city||"");a.find("#ship-state").val(b.state||"")}})})},verifyUserInfo:function(){$.qdAjax({url:"/no-cache/profileSystem/getProfile",dataType:"json",success:function(a){$.qdAjax({url:"/api/checkout/pub/profiles?email="+a.Email,dataType:"json",success:function(b){b.availableAddresses.length&&b.userProfile?($(".panel-group .panel:first-child").addClass("qd_step_done").find("#collapsePreRegister .panel-body").empty().html('<h4 class="center">Etapa concluida!</h4>').addClass("bg-success"),
$(".panel-group .panel #collapseCreateList").addClass("in")):(b=$(".panel-group .panel #collapsePreRegister"),"M"==a.Gender?a.Gender="male":"F"==a.Gender&&(a.Gender="female"),b.find("input[name=qd_form_email]").val(a.Email),b.find("input[name=qd_form_name]").val(a.FirstName||""),b.find("input[name=qd_form_lastname]").val(a.LastName||""),b.find("select[name=qd_form_gender]").val(a.Gender||""),List.savePreRegister(a.Email,a.UserId))}})}})},savePreRegister:function(a,b){function c(a){var b=$(".panel-group .panel #collapsePreRegister");
f=$(a).find("input#VTEXRequestVerificationToken").val()||"";b.find("input[name=qd_form_cpf]").val($(a).find("#editar-perfil input#document").val()||"");b.find("input[name=qd_form_phone]").val($(a).find("#editar-perfil input#homePhone").val()||"");List.applyMasks()}var d=$(".panel-group .panel #collapsePreRegister"),f;$.ajax({url:"/account",dataType:"html"}).done(function(a){c(a)}).error(function(a,b,d){c(a.responseText)});$(".giftlist-qd-v1-form-submit").on("click",function(c){c.preventDefault();
var e=!1;d.find("input.required:not([disabled]), select").each(function(){0==$(this).val().length&&($(this).focus(),e=!0)});e||(profileData={email:a,userId:b,VTEXRequestVerificationToken:"",firstName:d.find("input[name=qd_form_name]").val(),lastName:d.find("input[name=qd_form_lastname]").val(),document:d.find("input[name=qd_form_cpf]").val(),homePhone:d.find("input[name=qd_form_phone]").val(),gender:d.find("select[name=qd_form_gender]").val(),businessPhone:"",isCorporate:"False",nickName:"",birthDate:""},
shippingData={userId:b,postalCode:d.find("input[name=qd_form_zipcode]").val(),street:d.find("input[name=qd_form_street]").val(),number:d.find("input[name=qd_form_number]").val(),complement:d.find("input[name=qd_form_complement]").val()||"",neighborhood:d.find("input[name=qd_form_neighboor]").val(),city:d.find("input[name=qd_form_city]").val(),state:d.find("input[name=qd_form_state]").val(),receiverName:profileData.firstName,addressName:"Endereco",addressType:"1",reference:"",country:"BRA",addressId:""},
h(profileData,shippingData))});var h=function(a,b){var c=0;a.VTEXRequestVerificationToken=f;c++;$.ajax({url:"/no-cache/account/profile/save",type:"POST",data:a}).complete(function(){c--;0==c&&location.reload()});c++;$.ajax({url:"/no-cache/account/address/save",type:"POST",data:b}).complete(function(){c--;0==c&&location.reload()})}},fillAddressInfo:function(){var a=$(".panel-group .panel #collapsePreRegister .form-qd-v1-second-wrap"),b=a.find("input[name=qd_form_zipcode]");b.keyup(function(c){9>(b.val().replace("_",
"")||"").length||$.ajax({url:"/api/checkout/pub/postal-code/BRA/"+b.val(),dataType:"json",success:function(b){a.find("input[name=qd_form_street]").val(b.street||"");a.find("input[name=qd_form_neighboor]").val(b.neighborhood||"");a.find("input[name=qd_form_city]").val(b.city||"");a.find("input[name=qd_form_state]").val(b.state||"");a.find("[disabled], .enabled").each(function(){0==$(this).val().length?$(this).removeAttr("disabled").addClass("enabled"):$(this).attr("disabled","true").removeClass("enabled")})},
error:function(){a.find("*[disabled]").each(function(){$(this).removeAttr("disabled")})}})})},applyMasks:function(){$("#qd_form_cpf").mask("999.999.999-99");$("#qd_form_phone").mask("(99) 9999-9999");$("#qd_form_zipcode").mask("99999-999")}};
