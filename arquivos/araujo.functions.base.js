		...
		var execValidate = function(){
			...
			wrapper.find("form").validate({
				submitHandler: function(form){
					...
					$.ajax({
						url: "http://www.quatrodigital.com.br/clientes/araujo/encomenda.php",
						data: $form.serialize(),
						dataType: "jsonp",
						success: function(){
	...
	trackingHistory: function() {
		$(".trackingInfo").each(function() {
			...
			if ((/^[a-z]{2}[0-9]{9}$/i).test(tracking))
				wrapper.append('<iframe src="http://websro.correios.com.br/sro_bin/txect01$.QueryList?P_LINGUA=001&P_TIPO=002&P_COD_LIS=' + tracking + 'BR" frameborder="0" class="qd-shipping-tracking"></iframe>');
		});
	}
};
