var QdPbm = {
	...
	sever: 'http://chadebebe.araujo.com.br/araujo-pbm',
	pbmConsult: function() {
		...
		wrapper.find('button').click(function() {
			...
			$.qdAjax({
				...
				success: function() {QdPbm.pbmLoading.hide()}
			}).done(function(data) {
				QdPbm.pbmProcessItem(0, data[0].items, cpf, buyBtns, skuLists);
			});
		});
	},
	pbmProcessItem: function(ndx, items, cpf, buyBtns, skuLists) {
		if(typeof items[ndx] == 'undefined')
			return;
		...
		var price = 0;
		var sellingPrice = 0;
		for(var s = 0; s < items[ndx].sellers.length; s++){
			if(items[ndx].sellers[s].sellerId != 1)
				continue;

			sellingPrice = Math.round(items[ndx].sellers[s].commertialOffer.Price * 100);
			price = Math.round(items[ndx].sellers[s].commertialOffer.ListPrice * 100);
		}
		...
		var reqData = {
			...
			price: price,
			qtt: buyBtns.filter('[href*="sku=' + items[ndx].itemId + '&"]').closest(skuLists).find('.qd-sq-quantity').val() || 1,
			bDate: '',
			ean: items[ndx].ean
		};
		...
		.done(function(data) {
			...
			if(data.redirect && data.redirect.indexOf('ATIVA') == 0){
				var modal = $('.qd-common-modal').clone().removeClass('qd-common-modal');
				modal.modal();
			...
			if(data.item){
				if(data.item.QtdeAuth != reqData.qtt)	
					wrapper.append('<p>Quantidade solicitada é diferente da quantidade na qual foi concedido o desconto</p>');

				var priceDiscount = Math.round(price * ((100- data.item.DescPerc /100)/100));
				if(sellingPrice <= priceDiscount)
					wrapper.append('<p>O desconto oferecido para o item “' + items[ndx].nameComplete + '” é menor do que o preço oferecido pela Araujo. Fique tranquilo, você já esta pagando o menor preço para este item.</p>');
				else
					wrapper.append('<p>O item “' + items[ndx].nameComplete + '” teve o desconto concedido. Você pagará R$ ' + qd_number_format(priceDiscount / 100, 2, ",", ".") + '</p>');
		...
		function callNext() {
			QdPbm.pbmProcessItem(ndx + 1, items, cpf, buyBtns, skuLists);
		};
	},
	pbmRegexCache: {
		onlyNumber: /[^0-9]+/g,
