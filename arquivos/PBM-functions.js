var QdPbm = {
	init: function() {
		...
		// Caso o produto esteja indisponível, não faço nada
	...
	pbmConsult: function() {
		...
		wrapper.find('form').submit(function(e) {
			...
			var cpf = (cpfInput.val() || '').trim().replace(QdPbm.pbmRegexCache.onlyNumber, '');
	...
	pbmInsertSignUpForm: function(modal, data, cpf) {
		...
		form.find('.qd-pbm-ATIVA').hide().filter('.qd-pbm-' + data.redirect).show().find('input:not("[type=checkbox], [name=complement]"), input[name=accept], select').addClass('required');
		...
		form.validate({
			...
			submitHandler: function(form){
				...
				.fail(function() {
					$form.find('.qd-pbm-modal-response').html('<p class="bg-danger" style="display:none">Desculpe, ocorreu um erro inesperado! Por favor tente novamente e caso o erro persista entre em contato com o Atendimento ao Cliente.</p>').find('p').slideDown();
				})
				.done(function(data) {
					if(data.statusServico <= 19){
						modal.on('hidden.bs.modal', function() {$('.product-qd-v1-pbm button').click(); });
						modal.modal('hide');
					}
					else
						$form.find('.qd-pbm-modal-response').html('<p class="bg-danger" style="display:none">Ocorreu um problema ao enviar seus dados, veja a mensagem do sistema de PBM: <strong>“' + data.info + '”</strong>.</p>').find('p').slideDown();
				});
